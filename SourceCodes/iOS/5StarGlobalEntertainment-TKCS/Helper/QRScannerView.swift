...
public class QRScannerView: UIView {
    ...
    // MARK: - Public function for focus on QR-Code
    /// set the animation duration and image padding and for blured background
...
// MARK: - Create function for start scan
    ...
    // MARK: - Create function for Stop scan
...
// MARK: - Create function for Re-scan a ticket
    public func rescan() {
        guard isAuthorized() else { return }
        ...
        UIView.animate(withDuration: animationDuration, animations: { [weak self] in
            ...
            strongSelf.qrCodeImageView.center = center
                ...
                if strongSelf.isBlurEffectEnabled {
                    strongSelf.blurEffectView.isHidden = true // false
                }
...
extension QRScannerView: AVCaptureVideoDataOutputSampleBufferDelegate {
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        DispatchQueue.main.async {
            ...
            let x = 25
            let y = self.bounds.height * 0.191
            self.focusImageView = UIImageView(frame: CGRect(x: CGFloat(x), y: y, width: width, height: width))
            self.focusImageView.image = qrCodeImage
